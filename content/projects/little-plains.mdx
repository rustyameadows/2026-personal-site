...
export const project = {
  ...
  featured: true,
  description:
    "I joined Little Plains to build an agentic chat experience. The visual-retrieval system that agent needed became Explore. That breakthrough helped turn two years of ideas, archives, and experiments into a website the team could finally ship.",
...
<CaseStudyFacts
  items={[
    {
      label: "Role",
      value: "Product prototyping, AI systems, and engineering leadership"
    },
    {
      label: "Timeline",
...
The early agent checked Sanity and other sources live, even for basic questions that appeared again and again. That made simple answers unnecessarily slow. Many of the questions were variations of the same small set, so I started making compact, reviewed fact sheets. They were cheat sheets that kept common studio and project knowledge close at hand. I iterated through several versions as we learned what the agent needed to answer quickly and accurately.
...
Fast knowledge handled stable facts and common questions. Deep knowledge preserved the source material for questions that deserved real investigation. Model-proposed changes could also be reviewed before publication. A meeting note or stray document could not silently become the studio's public position.
...
**Explore** searches individual visual artifacts using language, color, or a reference image. Every result links back to its source project.
...
**Voice** uses the same context. An early voice experiment already existed when I joined; I rebuilt it through at least five substantial iterations to reach the integrated version on the site. It is still evolving, but it belongs to the same conversation as Ask.

The modes eventually came together in the OmniBar, a persistent object that lets the visitor move among navigation, visual search, typing, and speech. The initial idea to combine those surfaces came from the design team. I built an early pass at much of its layout, drawer, options, and behavior, then worked with the frontend team as they refined the transitions and interface details. That happened throughout the project. An idea became working software, then got sharper as the team used it.
...
My most useful contribution was often to give an unresolved idea a form that everybody could react to. We used working prototypes as design material. A conversation about how navigation and chat might coexist could become several functional variants. Search relevance could be judged against real media instead of a mock result set. Voice behavior could be tested with interruption, latency, and context intact. Working versions focused the discussion. The team could react to actual behavior instead of debating an abstraction.

I used Codex and the OpenAI API aggressively in both the product and the work of building it. They helped me move between system architecture, interface experiments, repetitive production work, evaluation, and implementation quickly enough to keep ideas alive while the team was still discussing them.
...
I generated each final color separately and turned it into sixteen baked painted states, for 192 frames across the family. Every sequence starts on the exact resting image, and I corrected drift in Green, Mint, and Cyan with measured whole-pixel translations, without regenerating, scaling, blurring, simulating, or morphing anything at runtime. The painted material is generated. The geometry and playback are deterministic.

The implementation is tightly controlled too. Hover provides local feedback but never performs a search. A committed color holds its deepest painted state. Touch and reduced-motion users receive stable states, and a missing animation falls back to the original mark. Beneath the painted surface, an authored group of results appears immediately and visual retrieval extends it.

The result is a small control that does a real job. AI made the painted material possible. I selected the direction, corrected the frames, and defined exactly how the control behaves.
...
The team participated heavily in QA and in shaping the final interface. My responsibility was to get a Next.js public site, a Rails retrieval and knowledge system, real-time voice, model-backed workflows, Sanity content, and frontend work from several people into one release.

Shipping mattered, especially after two years of the website feeling possible without feeling inevitable. I also learned how to work across product, design, and engineering inside a bigger team without treating them as separate phases. I could bring a backend diagnostic into a product conversation, turn visual experimentation into accessible interface behavior, and use evaluation to shape the design instead of grading it afterward.

I joined to build a chatbot. We shipped a website that could look at its own work because working software gave the team something concrete to rally around.
